...
07/11/22: How do ChIP/Input values of k-mers compare? Are replicates closer together than strains?

```{r}
B6_kmers_subset <- B6_kmers[,c(2,9,10)]
CAST_kmers_subset <- CAST_kmers[,c(2,11,12,13)]
LEWES_kmers_subset <- LEWES_kmers[,c(2,11,12,13)]
PWK_kmers_subset <- PWK_kmers[,c(2,11,12,13)]

PCA_merge <- merge(B6_kmers_subset,CAST_kmers_subset, by = 'kmer', all = TRUE)
PCA_merge <- merge(PCA_merge,LEWES_kmers_subset, by = 'kmer', all = TRUE)
PCA_merge <- merge(PCA_merge,PWK_kmers_subset, by = 'kmer', all = TRUE)

```

Make a PCA with aunadnace of all k-mers present in all strains

```{r}

rownames(PCA_merge) <- PCA_merge[,1]
PCA_merge <- PCA_merge[,2:12]
PCA_merge[is.na(PCA_merge)] <- 0
tPCA_merge <- t(PCA_merge)
...
PCA_merge_prcomp  <- prcomp(na.omit(tPCA_merge), scale = TRUE)

summary(PCA_merge_prcomp)

pcadf <- as.data.frame(PCA_merge_prcomp$x)
pcadf$SAMPLE <- rownames(pcadf)
pcadf$STRAIN <- str_split_fixed(pcadf$SAMPLE, "_", 4)

ggplot(pcadf) +
  geom_point(aes(PC1,PC2, color = c("grey","grey","green","green","green","purple","purple","purple","red","red","red"))) +
  theme_classic() +
...
##PC2 and PC3
ggplot(pcadf) +
  geom_point(aes(PC2,PC3, color = c("grey","grey","green","green","green","purple","purple","purple","red","red","red"))) +
  theme_classic() +
  theme(legend.position="none") +
  geom_text_repel(aes(PC1, PC2, label= SAMPLE)) 
```
...
```{r}
PCA_merge_prcomp$sdev^2 / sum(PCA_merge_prcomp$sdev^2)

```
multiply by 100 for percent
[1] 3.000471e-01 2.533840e-01 1.791313e-01 6.308201e-02 4.686300e-02
 [6] 4.036183e-02 3.610783e-02 3.052863e-02 2.586179e-02 2.463254e-02
[11] 1.065787e-26

Drawing 90% connfidence elipses around replicates
DID NOT WORK
```{r}
library(FactoMineR)

tPCA_merge$SAMPLE <- rownames(tPCA_merge)
tPCA_merge$STRAIN <- str_split_fixed(tPCA_merge$SAMPLE, "_", 4)

res.pca <- PCA(tPCA_merge, scale.unit=T, ncp=5, quali.sup = 26, graph = F)
concat = cbind.data.frame(tPCA_merge[,26], res.pca$ind$coord)
...
B6_kmers$ChIPoverInputSD <- rowSds(as.matrix(B6_kmers[,9:10]))
...
```{r}
ggplot(B6all, aes(x = log10(INPUT), y = log10(CENPA), color = REPLICATE)) +
  geom_jitter(alpha = 0.5)
```
...
ggplot(B6_kmers, aes(x = B6_F_092920_CENPAoverInput, y = B6_M_CENPAoverInput)) +
  ...
  ylim(0,2500)
```
Only plot k-mers in all replicates
I did this in the first place!! nothing changed
don't run
```{r}
B61 <- B6_kmers[,c(2,3,6,12)]
B61$REPLICATE <- c("1")
...
B6 <- merge(B61,B62,by = "KMER", all = FALSE)
B61_merge <- B6[,c(1,2,3,4,5)]
colnames(B61_merge) <- c("KMER","CENPA","INPUT","RANK","REPLICATE")
B62_merge <- B6[,c(1,6,7,8,9)]
colnames(B62_merge) <- c("KMER","CENPA","INPUT","RANK","REPLICATE")

B6all <- rbind(B61_merge,B62_merge)
ggplot(B6all, aes(x = INPUT, y = CENPA, color = REPLICATE, shape = RANK)) +
  geom_jitter()
```
...
##########
Map k-mer to consensus sequence and remake previous plot and make coverage plot
Convert k31txt to fastq with python script

```{python}
#!/urs/bin/env python

###Change input and output path###
pathinput='/fastscratch/uma/enriched_kmers_centromere_mapping/PWK.0.1percent.TopEnrich.k31mers.txt'
pathoutput='/fastscratch/uma/enriched_kmers_centromere_mapping/PWK.0.1percent.TopEnrich.k31mers.txt.fastq'
...
Map fastq to centromere consensus
```{bash}
#!/bin/sh
...
SRA=$(sed -n "${SLURM_ARRAY_TASK_ID}p" /fastscratch/uma/enriched_kmers_centromere_mapping/samples_list.txt)
REF='/projects/dumont-lab/uma/centromere_mapping/CENPA_ChIP'
INPUT='/fastscratch/uma/enriched_kmers_centromere_mapping'
OUTPUT='/fastscratch/uma/enriched_kmers_centromere_mapping'

singularity exec /home/arorau/bwa/bwa.sif bwa aln -n 4 ${REF}/centromere_consensus.fa ${INPUT}/${SRA} > ${OUTPUT}/${SRA}.sai

singularity exec /home/arorau/bwa/bwa.sif bwa samse ${REF}/centromere_consensus.fa ${OUTPUT}/${SRA}.sai ${INPUT}/${SRA} > ${OUTPUT}/${SRA}.sam

singularity exec /home/arorau/samtools/samtools.sif samtools view -S -F4 ${OUTPUT}/${SRA}.sam > ${OUTPUT}/${SRA}.mapped.sam

singularity exec /home/arorau/samtools/samtools.sif samtools sort -o ${OUTPUT}/${SRA}.bam ${OUTPUT}/${SRA}.sam

singularity exec /home/arorau/samtools/samtools.sif samtools index -@ 16 ${OUTPUT}/${SRA}.sorted.bam

singularity exec /home/arorau/samtools/samtools.sif samtools idxstats ${OUTPUT}/${SRA}.sorted.bam > ${OUTPUT}/${SRA}.idxstats.txt
```
...
PWK: 667/3617
```{r}
setwd("/projects/dumont-lab/uma/CENPA_ChIP/k31txt")
B6_mappedkmers <- read.delim(file = "B6.0.1percent.TopEnrich.k31mers.txt.fastq.mapped.sam", header = FALSE)
B6_mappedkmers$strain <- c("C57BL/6J")
B6_mappedkmers %>% 
  group_by(V3, V4,strain) %>%
  dplyr::summarise(count = n()) %>%
  arrange(-count) -> B6_mappedkmers_count

B6_mappedkmers %>% 
  group_by(V3, V13, strain) %>%
  dplyr::summarise(count = (n()/3314)*100) -> B6_mappedkmers_mismatches


CAST_mappedkmers <- read.delim(file = "CAST.0.1percent.TopEnrich.k31mers.txt.fastq.mapped.sam", header = FALSE)
CAST_mappedkmers$strain <- c("CAST/EiJ")
CAST_mappedkmers %>% 
  group_by(V3, V4,strain) %>%
  dplyr::summarise(count = n()) %>%
  arrange(-count) -> CAST_mappedkmers_count

CAST_mappedkmers %>% 
  group_by(V3, V13, strain) %>%
  dplyr::summarise(count = (n()/3823)*100) -> CAST_mappedkmers_mismatches

LEWES_mappedkmers <- read.delim(file = "LEWES.0.1percent.TopEnrich.k31mers.txt.fastq.mapped.sam", header = FALSE)
LEWES_mappedkmers$strain <- c("LEWES/EiJ")
LEWES_mappedkmers %>% 
  group_by(V3, V4,strain) %>%
  dplyr::summarise(count = n()) %>%
  arrange(-count) -> LEWES_mappedkmers_count

LEWES_mappedkmers %>% 
  group_by(V3, V13, strain) %>%
  dplyr::summarise(count = (n()/4260)*100) -> LEWES_mappedkmers_mismatches

PWK_mappedkmers <- read.delim(file = "PWK.0.1percent.TopEnrich.k31mers.txt.fastq.mapped.sam", header = FALSE)
PWK_mappedkmers$strain <- c("PWK/PhJ")
PWK_mappedkmers %>% 
  group_by(V3, V4,strain) %>%
  dplyr::summarise(count = n()) %>%
  arrange(-count) -> PWK_mappedkmers_count

PWK_mappedkmers %>% 
  group_by(V3, V13, strain) %>%
  dplyr::summarise(count = (n()/3617)*100) -> PWK_mappedkmers_mismatches


all_mappedkmers_count <- rbind(B6_mappedkmers_count,CAST_mappedkmers_count,LEWES_mappedkmers_count,PWK_mappedkmers_count)
all_mappedkmers <- rbind(B6_mappedkmers,CAST_mappedkmers,LEWES_mappedkmers,PWK_mappedkmers)
all_mappedkmers_mismatches <- rbind(B6_mappedkmers_mismatches,CAST_mappedkmers_mismatches,LEWES_mappedkmers_mismatches,PWK_mappedkmers_mismatches)

```

```{r}
all_mappedkmers_mismatches %>%
  filter(V3 == "MinorSatelliteConsensusWongandRattner1988x3") %>%
  ggplot(aes(x = strain, y = count, fill = strain)) +
  geom_col() +
  theme_classic() +
  facet_wrap(vars(V13), ncol=5, scales = "free_x") +
  scale_fill_manual(values = c("black","#228b22","#a020f0","#ff0000")) +
  theme(axis.text.x=element_text(angle=45, hjust =1))
```


collapse onto 120 bp repeat
```{r}
...
Collapse repeats
```{r}
all_mappedkmers_count %>%
  filter(V3 == 'MinorSatelliteConsensusWongandRattner1988x3') -> all_mappedkmers_count_minor

for (i in 1:nrow(all_mappedkmers_count_minor)) {
  if (all_mappedkmers_count_minor[i,2] >240) {
    all_mappedkmers_count_minor[i,2] <- all_mappedkmers_count_minor[i,2] - 240
  }
  if (all_mappedkmers_count_minor[i,2] >120) {
    all_mappedkmers_count_minor[i,2] <- all_mappedkmers_count_minor[i,2] - 120
  }
}
```

```{r}
all_mappedkmers_count_minor %>%
  group_by(V3, strain) %>%
  dplyr::summarise(sum(count)) 
```
MinorSatelliteConsensusWongandRattner1988x3	C57BL/6J	2593	
MinorSatelliteConsensusWongandRattner1988x3	CAST/EiJ	1569	
MinorSatelliteConsensusWongandRattner1988x3	LEWES/EiJ	3303	
MinorSatelliteConsensusWongandRattner1988x3	PWK/PhJ	1284	

normalize
```{r}
B6norm <- all_mappedkmers_count_minor[all_mappedkmers_count_minor$strain == "C57BL/6J",]
B6norm$count <- ((B6norm$count/2593) * 100)
CASTnorm <- all_mappedkmers_count_minor[all_mappedkmers_count_minor$strain == "CAST/EiJ",]
CASTnorm$count <- ((CASTnorm$count/1569) * 100)
LEWESnorm <- all_mappedkmers_count_minor[all_mappedkmers_count_minor$strain == "LEWES/EiJ",]
LEWESnorm$count <- ((LEWESnorm$count/3303) * 100)
PWKnorm <- all_mappedkmers_count_minor[all_mappedkmers_count_minor$strain == "PWK/PhJ",]
PWKnorm$count <- ((PWKnorm$count/1284) * 100)

all_mappedkmers_count_minor_norm <- rbind(B6norm,CASTnorm,LEWESnorm,PWKnorm)
```
...
all_mappedkmers_count_minor %>%
  group_by(V3, V4,strain) %>%
  dplyr::summarise(count_agg = sum(count)) %>%
  ggplot(aes(x = V4, y = count_agg, color = strain)) +
  geom_line() +
...
Normalized to number of kmers that mapped to consensus in each strain
```{r}
all_mappedkmers_count_minor_norm %>%
  group_by(V3, V4,strain) %>%
  dplyr::summarise(count_agg = sum(count)) %>%
  ggplot(aes(x = V4, y = count_agg, color = strain)) +
  geom_line() +
...
###RE RUN READ SCORE RANKING with all = FALSE
Read score for each strains enriched k-mers
B6_F_092920_CENPA
B6_M_CENPA
CAST_F_011921_CENPA
...
write.table(B6ordered, file = "B6_0.1percent_allFALSE_normReadcount_normReadscore.txt", row.names = FALSE)

B6top1000 <- B6ordered[1:1000,]

write.table(B6top1000, file = "B6_0.1percent_allFALSE_normReadcount_normReadscore_top1000.txt", row.names = FALSE)
...
####
setwd("/projects/dumont-lab/uma/CENPA_ChIP/readscore")
B6 <- read.table(file = "B6_0.1percent_allFALSE_normReadcount_normReadscore.txt", header = TRUE)
#sort by AVGcount then AVGreadscore
B6ordered <- B6[order(-B6[,8], -B6[,9] ),]

write.table(B6ordered, file = "B6_0.1percent_allFALSE_normReadscore_normReadcount.txt", row.names = FALSE)

B6top1000 <- B6ordered[1:1000,]

write.table(B6top1000, file = "B6_0.1percent_allFALSE_normReadscore_normReadcount_top1000.txt", row.names = FALSE)
...
write.table(CASTordered, file = "CAST_0.1percent_allFALSE_normReadcount_normReadscore.txt", row.names = FALSE)

CASTtop1000 <- CASTordered[1:1000,]

write.table(CASTtop1000, file = "CAST_0.1percent_allFALSE_normReadcount_normReadscore_top1000.txt", row.names = FALSE)
...
####
setwd("/projects/dumont-lab/uma/CENPA_ChIP/readscore")
CAST <- read.table(file = "CAST_0.1percent_allFALSE_normReadcount_normReadscore.txt", header = TRUE)
#sort by AVGcount then AVGreadscore
CASTordered <- CAST[order(-CAST[,11], -CAST[,12] ),]

write.table(CASTordered, file = "CAST_0.1percent_allFALSE_normReadscore_normReadcount.txt", row.names = FALSE)

CASTtop1000 <- CASTordered[1:1000,]

write.table(CASTtop1000, file = "CAST_0.1percent_allFALSE_normReadscore_normReadcount_top1000.txt", row.names = FALSE)
...
write.table(LEWESordered, file = "LEWES_0.1percent_allFALSE_normReadcount_normReadscore.txt", row.names = FALSE)

LEWEStop1000 <- LEWESordered[1:1000,]

write.table(LEWEStop1000, file = "LEWES_0.1percent_allFALSE_normReadcount_normReadscore_top1000.txt", row.names = FALSE)
...
####
setwd("/projects/dumont-lab/uma/CENPA_ChIP/readscore")
LEWES <- read.table(file = "LEWES_0.1percent_allFALSE_normReadcount_normReadscore.txt", header = TRUE)
#sort by AVGcount then AVGreadscore
LEWESordered <- LEWES[order(-LEWES[,11], -LEWES[,12] ),]

write.table(LEWESordered, file = "LEWES_0.1percent_allFALSE_normReadscore_normReadcount.txt", row.names = FALSE)

LEWEStop1000 <- LEWESordered[1:1000,]

write.table(LEWEStop1000, file = "LEWES_0.1percent_allFALSE_normReadscore_normReadcount_top1000.txt", row.names = FALSE)
...
write.table(PWKordered, file = "PWK_0.1percent_allFALSE_normReadcount_normReadscore.txt", row.names = FALSE)

PWKtop1000 <- PWKordered[1:1000,]

write.table(PWKtop1000, file = "PWK_0.1percent_allFALSE_normReadcount_normReadscore_top1000.txt", row.names = FALSE)
```

```{r}
####
setwd("/projects/dumont-lab/uma/CENPA_ChIP/readscore")
PWK <- read.table(file = "PWK_0.1percent_allFALSE_normReadcount_normReadscore.txt", header = TRUE)
#sort by AVGcount then AVGreadscore
PWKordered <- PWK[order(-PWK[,11], -PWK[,12] ),]

write.table(PWKordered, file = "PWK_0.1percent_allFALSE_normReadscore_normReadcount.txt", row.names = FALSE)

PWKtop1000 <- PWKordered[1:1000,]

write.table(PWKtop1000, file = "PWK_0.1percent_allFALSE_normReadscore_normReadcount_top1000.txt", row.names = FALSE)
